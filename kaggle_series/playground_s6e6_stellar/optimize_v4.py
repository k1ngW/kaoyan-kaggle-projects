"""
S6E6 v4 — Best models voting (no CatBoost) + more estimators
"""
...
all_data['brightness_sum']=all_data[bands].sum(axis=1); all_data['brightness_std']=all_data[bands].std(axis=1)
...
models = {
    'XGB1': xgb.XGBClassifier(n_estimators=500,learning_rate=0.04,max_depth=7,subsample=0.8,colsample_bytree=0.7,random_state=42,verbosity=0,n_jobs=-1),
    'XGB2': xgb.XGBClassifier(n_estimators=500,learning_rate=0.03,max_depth=8,subsample=0.7,colsample_bytree=0.6,random_state=43,verbosity=0,n_jobs=-1),
    'LGB1': lgb.LGBMClassifier(n_estimators=500,learning_rate=0.04,max_depth=7,subsample=0.8,colsample_bytree=0.7,random_state=42,verbose=-1,n_jobs=-1),
    'LGB2': lgb.LGBMClassifier(n_estimators=500,learning_rate=0.03,max_depth=9,subsample=0.7,colsample_bytree=0.6,random_state=43,verbose=-1,n_jobs=-1),
...
print(f"  Ensemble CV: {s.mean():.5f}")

vote.fit(X,y_enc); preds=vote.predict(X_test)

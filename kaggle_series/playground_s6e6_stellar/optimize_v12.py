"""
S6E6 v12 — v8 XGB + CatBoost 软投票
=====================================
策略: v8同款特征 + CatBoost GPU → 与v8 XGB OOF软投票 → 冲Silver
预计: 30 min, LB 0.9685+
"""
...
if __name__=='__main__':
    ...
    print(f"  特征: {X.shape[1]} 列, TE列: {len(TE_COLS)}, 模型类别列: {len(MC)}")
    del all_df,trb,teb,orb,train,test,orig; gc.collect()

    # Load v8 XGB OOF
    v8_xgb_oof=np.load(OOF_DIR/"v8_oof_preds.npy"); v8_xgb_test=np.load(OOF_DIR/"v8_test_preds.npy")
    v8_labels=np.load(OOF_DIR/"v8_oof_labels.npy")
    print(f"  加载 v8 XGB OOF: BAcc {balanced_accuracy_score(v8_labels,v8_xgb_oof.argmax(axis=1)):.6f}")

    # Train CatBoost only
    print(f"\n{'='*60}"); print("  Step 2/3: CatBoost GPU 5-Fold CV"); print("="*60)
    ...
    for fold,(tr_idx,va_idx) in enumerate(skf.split(np.zeros(len(y_np),dtype=np.int8),y_np),start=1):
        ...
        model=cb.CatBoostClassifier(loss_function='MultiClass',num_boost_round=3000,learning_rate=0.02,depth=7,
                                     l2_leaf_reg=4.0,bootstrap_type='Bernoulli',subsample=0.75,
                                     random_seed=fs,thread_count=-1,verbose=0,early_stopping_rounds=100,
                                     task_type='GPU',devices='0')
        model.fit(X_tr_a,y_tr.values,sample_weight=sw,eval_set=[(X_va_a,y_va.values)])
        cat_oof[va_idx]=model.predict_proba(X_va_a).astype('float32')
        cat_tst+=model.predict_proba(X_te_a).astype('float32')/N_SPLITS
        bacc=balanced_accuracy_score(y_va,cat_oof[va_idx].argmax(axis=1))
        print(f" BAcc: {bacc:.6f}")
        del X_tr,X_va,X_te,X_tr_a,X_va_a,X_te_a,model; gc.collect()

    cat_score=balanced_accuracy_score(y_np,cat_oof.argmax(axis=1))
    print(f"  CatBoost OOF: {cat_score:.6f}")

    # Soft Vote
    print(f"\n{'='*60}"); print("  Step 3/3: 软投票 + 提交"); print("="*60)
    vote_oof=(v8_xgb_oof+cat_oof)/2; vote_tst=(v8_xgb_test+cat_tst)/2
    ...
    np.save(OOF_DIR/'v12_cat_oof.npy',cat_oof.astype('float32'))
    np.save(OOF_DIR/'v12_cat_test.npy',cat_tst.astype('float32'))
    np.save(OOF_DIR/'v12_vote_oof.npy',vote_oof.astype('float32'))
    np.save(OOF_DIR/'v12_vote_test.npy',vote_tst.astype('float32'))

    from experiment_logger import log_experiment
    log_experiment('v12',{'xgb':float(balanced_accuracy_score(v8_labels,v8_xgb_oof.argmax(axis=1))),'cat':float(cat_score),'vote':float(vote_score)},notes='v8 XGB+Cat soft vote')

    print(f"  ✅ v12 Complete! Vote OOF: {vote_score:.6f}")
    print(f"{'='*60}")

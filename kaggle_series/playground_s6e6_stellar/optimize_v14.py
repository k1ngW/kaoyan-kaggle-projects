"""
S6E6 v14 — RealMLP 256 epoch + v8 XGB 软投票
===============================================
RealMLP_TD(pytabkit) 256 epoch / 171特征 / GPU
训练完与 v8 XGB OOF 软投票 → 冲 Top 15%

用法: C:/Users/Lenovo/kaggle_env/python -s -u optimize_v14.py
预计: 40-60 min
"""
...
if __name__=='__main__':
    print("="*60)
    print("  v14: RealMLP 256 epoch + v8 XGB 软投票")
    print("="*60)
    ...
    # Load v8 XGB OOF
    v8_oof=np.load(OOF_DIR/"v8_oof_preds.npy"); v8_tst=np.load(OOF_DIR/"v8_test_preds.npy")
    v8_labels=np.load(OOF_DIR/"v8_oof_labels.npy")
    print(f"  加载 v8 XGB OOF: BAcc {balanced_accuracy_score(v8_labels,v8_oof.argmax(axis=1)):.6f}")

    # Train RealMLP 256 epoch
    print(f"\n{'='*60}")
    print("  Step 2/3: RealMLP 256 epoch GPU (5-fold CV)")
    print("="*60)
    y_np=y.values.astype(np.int32)
    skf=StratifiedKFold(n_splits=N_SPLITS,shuffle=True,random_state=SEED)
    rmlp_oof=np.zeros((len(X),NC),dtype='float32'); rmlp_tst=np.zeros((len(X_test),NC),dtype='float32')
    ...
    for fold,(tr_idx,va_idx) in enumerate(skf.split(np.zeros(len(y_np),dtype=np.int8),y_np),start=1):
        ...
        # 同 v8: Fold-safe TE + encoding + 171 features
        ...
        model=RealMLP_TD_Classifier(device='cuda',random_state=fs,n_epochs=256,batch_size=1024)
        model.fit(X_tr_s,y_tr.values)
        rmlp_oof[va_idx]=model.predict_proba(X_va_s).astype('float32')
        rmlp_tst+=model.predict_proba(X_te_s).astype('float32')/N_SPLITS
        bacc=balanced_accuracy_score(y_va,rmlp_oof[va_idx].argmax(axis=1))
        print(f" BAcc: {bacc:.6f}")
        del X_tr,X_va,X_te,X_tr_a,X_va_a,X_te_a,X_tr_s,X_va_s,X_te_s,model,scaler; gc.collect()

    rmlp_ba=balanced_accuracy_score(y_np,rmlp_oof.argmax(axis=1))
    print(f"  RealMLP OOF: {rmlp_ba:.6f}")

    # Soft Vote
    print(f"\n{'='*60}"); print("  Step 3/3: 软投票 + 提交"); print("="*60)
    vote_oof=(v8_oof+rmlp_oof)/2; vote_tst=(v8_tst+rmlp_tst)/2
    vote_ba=balanced_accuracy_score(y_np,vote_oof.argmax(axis=1))
    print(f"  v8 XGB    OOF: {balanced_accuracy_score(v8_labels,v8_oof.argmax(axis=1)):.6f}")
    print(f"  RealMLP   OOF: {rmlp_ba:.6f}")
    ...
    np.save(OOF_DIR/'v14_rmlp_oof.npy',rmlp_oof.astype('float32'))
    np.save(OOF_DIR/'v14_rmlp_test.npy',rmlp_tst.astype('float32'))
    np.save(OOF_DIR/'v14_vote_oof.npy',vote_oof.astype('float32'))
    np.save(OOF_DIR/'v14_vote_test.npy',vote_tst.astype('float32'))

    from experiment_logger import log_experiment
    log_experiment('v14',{'rmlp':float(rmlp_ba),'vote':float(vote_ba)},notes='RealMLP 256ep + v8 XGB vote')
    print(f"  ✅ v14 Complete! Vote OOF: {vote_ba:.6f}")
